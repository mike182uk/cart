...
class Cart
{
     ...
     *
     * @param  bool|string                                $id     ID assigned to this cart instance
     * @param  array                                      $config Configuration options associated with this cart
     * @throws \Cart\Exception\InvalidCartConfigException
     */
    public function __construct($id = false, $config)
    {
        ...
        if ( ! is_array($config)) {
            throw new InvalidCartConfigException(sprintf('Either no configuration options where passed to the cart: %s, or the configuration options were not formatted as an array', $this->id));
        } else {
     ...
     *
     * @param  string     $uid   Unique identifier of the item in the cart
     * @param  string     $key   Key of the value to be updated
     * @param  mixed      $value New value
     * @return bool|mixed If the fields other than the quantity are updated the UID is returned as it is regenerated,
     *                    if the item is updated and only the quantity is amended true is returned
     * @throws \Cart\Exception\InvalidCartItemException
     */
    public function update($uid, $key, $value)
    {
        if ($this->exists($uid)) {
            ...
            throw new InvalidCartItemException(sprintf('Cart item does not exist: %s in the cart instance: %s', $uid, $this->id));
        }
    }
     ...
     * @return float Yotal for the cart
     */
     ...
     *
     * @return float Yotal tax for the cart
     */
    public function tax()
    {
        return number_format(
     ...
     *
     * @param array $cart Data associated with the cart to be imported into the cart
     */
     ...
     * @param mixed  $value Meta data to be saved against the cart
     */
     ...
     *
     * @param  bool|string $key Key of the meta data item saved against cart
